...
const char* ssid     = "ZureWifia";
const char* password = "ZurePasahitza";
...
void setHTML()
{
  ...
  webString += "<h1>Lizardi-Oteitza IoT Termostatoa </h1>\n";
...
void setHTML()
{
  ...
  webString += "      width: 250, height: 150,";
  webString += "      min: -10, max: 50,";
  webString += "      yellowFrom: -10, yellowTo: " + String(heatOn) + ",";
  webString += "      greenFrom: " + String(heatOn) + ", greenTo: " + String(heatOff) + ",";
  webString += "      redFrom: " + String(heatOff) + ", redTo: 50,";
  webString += "       minorTicks: 5";
  webString += "    }; \n";
  webString += "   var chart = new google.visualization.Gauge(document.getElementById('chart_divTemp')); \n";
  webString += "  chart.draw(data, options); \n";
  ...
  webString += "<table style=\"width:800px;\"><tr><td>";
  webString += "<div>Hozgailua OFF: &le;" + String((int)heatOn) + "&deg;C</div>\n";
  webString += "<div>Hozgailua ON:  &ge;" + String((int)heatOff) + "&deg;C</div>\n";
  webString += "</td><td style=\"text-align:right\">";
  webString += "OFF T&deg: &le; <input type='text' value='" + String((int)heatOn) + "' name='heatOn' maxlength='3' size='2'><br>\n";
  webString += "ON T&deg: &ge; <input type='text' value='" + String((int)heatOff) + "' name='heatOff' maxlength='3' size='2'><br>\n";
  webString += "</td><td style=\"text-align:right\">";
  webString += "Laginketa maiztasuna (s): <input type='text' value='" + String((long)interval / 1000) + "' name='sRate' maxlength='3' size='2'><br>\n";
  webString += "Gehienezko Datu Sektorea: <input type='text' value='" + String((long)maxFileData) + "' name='maxData' maxlength='3' size='2'><br>\n";
...
void handle_submit() {
  ...
  if (server.args() > 0 ) {
      ...
      if (server.argName(i) == "heatOn") {
        ...
        if (server.arg(i) != "")
        {
          if (isValidNumber(server.arg(i)) )
            ...
            webMessage += "OFF tenperaturak zenbakia izan behar du<br>";
        }
        else
          webMessage += "OFF tenperatura behar da<br>";
      }
      ...
      if (server.argName(i) == "heatOff") {
        ...
        if (server.arg(i) != "")
        {
          if (isValidNumber(server.arg(i)) )
            ...
            webMessage += "ON tenperaturak zenbakia izan behar du<br>";

        }
        else
          webMessage += "ON tenperatura behar da<br>";
      }
    ...
    if (tempOFF <= tempON)
      webMessage += "ON Tenp OFF tenp baina txikiagoa izan behar du <br>";
